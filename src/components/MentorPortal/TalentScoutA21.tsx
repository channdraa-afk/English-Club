...
export const TalentScoutA21: React.FC<TalentScoutA21Props> = ({
  ...
  const handleRevokeStar = async (starId: string, memberName: string) => {
    const confirmDelete = window.confirm(
      `Apakah kamu yakin ingin mencopot bintang keaktifan untuk ${memberName} pada sesi ini?`
    );
    if (!confirmDelete) return;

    try {
      sound.playPop();
      await onRemoveStar(starId);
    } catch (err: any) {
      sound.playError();
      alert('Gagal mencopot bintang: ' + (err.message || 'Terjadi kesalahan'));
    }
  };
                ...
                <div className="flex items-center gap-2 shrink-0 self-end md:self-auto">
                  {sessionStar ? (
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        type="button"
                        onClick={() => handleRevokeStar(sessionStar.id, student.name)}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between pt-1">
                        ...
                        <button
                          type="button"
                          onClick={() => handleRevokeStar(star.id, historyMember.name)}
