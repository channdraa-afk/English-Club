...
export const ArenaPlayer: React.FC<ArenaPlayerProps> = ({ members, onBackToHome }) => {
  // Live Session & Leaderboard Loading State
  ...
  const handleJoin = async (e?: React.FormEvent, spectateOnly = false) => {
    ...
    try {
      ...
      if (existingSub) {
        // Already submitted: show completed screen immediately (Score & Leaderboard)
        sound.playSuccess();
        ...
        fetchLeaderboardData(sessionData.id);
        return;
      }

      // If user chose Spectator mode (only wants to see the leaderboard first without submitting yet)
      if (spectateOnly) {
        sound.playSuccess();
        setSession(sessionData);
        setQuiz(quizData);
      ...
      <div className="w-full max-w-md mx-auto px-4 py-8 animate-fade-in relative">
        ...
        <div className="bg-white rounded-3xl border-2 border-slate-200 shadow-[0_6px_0_0_#e2e8f0] p-6 space-y-4 animate-fade-in">
          <form onSubmit={handleJoin} className="space-y-4">
            ...
            {/* Action Buttons: Masuk Arena & Lihat Papan Skor Live */}
            <div className="space-y-2 pt-1">
              <TactileButton
                type="submit"
                variant="brand"
              ...
              <TactileButton
                ...
                variant="white"
                size="md"
                className="w-full text-slate-700 hover:text-amber-600 font-black border-2 border-slate-200"
                disabled={!tokenInput.trim() || !selectedMember || isJoining}
                onClick={() => handleJoin(undefined, true)}
              >
                <Trophy className="w-4 h-4 text-amber-500" />
                <span>Lihat Papan Skor Live Saja</span>
              </TactileButton>
            </div>
  ...
  if (isCompleted) {
    return (
      <div className="w-full max-w-lg mx-auto px-4 py-8 animate-fade-in space-y-6">
        {isSpectating ? (
          /* Spectator Card (Belum Mengerjakan) */
          <div className="bg-white rounded-3xl border-2 border-blue-400 shadow-[0_8px_0_0_#2563eb] p-6 text-center space-y-4">
            <div className="w-16 h-16 mx-auto rounded-3xl bg-blue-100 text-blue-700 flex items-center justify-center border-2 border-blue-300 shadow-[0_4px_0_0_#93c5fd]">
            ...
            <div>
              <span className="text-[10px] font-black uppercase tracking-wider text-blue-700 bg-blue-50 px-3 py-1 rounded-full border border-blue-200">
                Mode Penonton • Belum Mengerjakan
              </span>
            ...
            <p className="text-xs font-medium text-slate-600 leading-relaxed">
              Kamu telah login ke arena dengan Token <strong className="font-mono text-blue-700 font-black">{session?.room_code}</strong>. Kamu dapat memantau peringkat kuis secara live atau mulai berkompetisi sekarang!
            </p>
            ...
            <TactileButton
              ...
              onClick={() => {
                sound.playPop();
                if (session && selectedMember) {
                  const saveKey = `ec_arena_save_${session.id}_${selectedMember.id}`;
                  const rawSave = safeStorage.get(saveKey);
                  if (rawSave && quiz) {
                    try {
                      const parsed = JSON.parse(rawSave);
                      if (
                        ...
                        typeof parsed.currentIdx === 'number' &&
                        parsed.currentIdx > 0 &&
                        parsed.currentIdx < quiz.questions.length &&
                        Date.now() - (parsed.savedAt || 0) < 3 * 3600 * 1000
                      ) {
                        setPendingSavePoint({
                          sessionData: session,
        ...
        <div className="bg-white rounded-3xl border-2 border-slate-200 shadow-[0_6px_0_0_#e2e8f0] p-6 space-y-4">
          ...
          <div className="space-y-2 pt-2">
            <TactileButton
              variant="white"
              size="md"
              className="w-full text-slate-600 border-2 border-slate-200"
              onClick={() => {
                sound.playPop();
                setSession(null);
                setQuiz(null);
            ...
            >
              ...
              <span>Ganti Siswa / Keluar Arena</span>
            </TactileButton>
